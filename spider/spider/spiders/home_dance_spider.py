...
import re
class home_dance_spider(scrapy.Spider):

    name = "home_dance_spider"
    ...
    script = """
        function main(splash)
            ...
            splash:runjs("$('#index_nav > .nav-list > :nth-child(5)').click()")
            splash:wait(2)
            return {
                html = splash:html()
            }
    ...
    def __init__(self, **kwargs):
        super(home_dance_spider, self).__init__(**kwargs)
        print "启动爬虫"
    ...
    def parse(self, response):
        music_ul = response.xpath(
            "//div[@id='b_dance']//ul[@class='vidbox v-list']//li"
        )
        for sel in music_ul:
            item = HomeItem()
